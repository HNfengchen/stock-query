...
def calculate_macd(close_prices: Union[List, pd.Series], 
                   fast: int = 12, 
                   slow: int = 26, 
                   signal: int = 9) -> Dict:
    """
    ...
    dif_val = round(dif.iloc[-1], 4)
    dea_val = round(dea.iloc[-1], 4)
...
def calculate_rsi(close_prices: Union[List, pd.Series], 
                  periods: List[int] = [6, 12, 24]) -> Dict:
    """
    计算RSI指标
    ...
    for period in periods:
        if len(closes) < period + 1:
            result[f'RSI({period})'] = {'value': None, 'status': '数据不足'}
            continue
        ...
        result[f'RSI({period})'] = {'value': value, 'status': status}
    
    return result


def calculate_kdj(high_prices: Union[List, pd.Series], 
...
def calculate_ma(close_prices: Union[List, pd.Series], 
                 periods: List[int] = [5, 10, 20, 60]) -> Dict:
    """
    计算移动平均线
...
def calculate_boll(close_prices: Union[List, pd.Series], 
                   n: int = 20, 
                   k: int = 2) -> Dict:
    """
    计算布林带指标
...
def calculate_volume_ratio(volume: Union[List, pd.Series], 
                           n: int = 5) -> Dict:
    """
    计算量比指标
